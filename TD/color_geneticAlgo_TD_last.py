...
def onSetupParameters(scriptOp):
	init_pop = 255*op("init_pop").numpyArray()
	print(init_pop.shape)
	init_pop = init_pop.astype(np.uint8)
	scriptOp.store('last_pop',init_pop)
	scriptOp.copyNumpyArray(init_pop)
	return
...
def run_algorithm_step(pM, pC, pop_size, dna_size, goal, last_population):
	...
	last_population = np.array(next_hopefully_diverser_gen(last_population, pC, pM, dna_size, goal))
...
def onCook(scriptOp):
	...
	last_population_fetch = scriptOp.fetch('last_pop',op("init_pop").numpyArray())

	last_population = run_algorithm_step(pM, pC, pop_size, dna_size, goal, last_population_fetch)

	out_array = np.ones((dna_size,pop_size,4), dtype = np.uint8)
	print(out_array[:,:,0].shape)
	print(last_population)
	out_array[:,:,0] = last_population[:,:,0]
	out_array[:,:,1] = last_population[:,:,0]
	out_array[:,:,2] = last_population[:,:,0]

	scriptOp.store('last_pop', last_population)
